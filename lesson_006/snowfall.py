...
def fallen_snowflakes(new_list):  # А вот здесь и далее, убрать new_list и count не получается...
    ...
    # TODO Что-то все равно не понятно. Count-ом у меня добавляются новые снежинки в список, получается нельзя убирать
    # TODO его полностью.
    # TODO А как только я пробую убрать new_list из параметров в глобальную переменную - он начинает распухать...
